...
var cityModel = function () {
	...
	var options = {
		...
		port: '80',
		path: 'dummy',  // actual path value is set in getCityWeather()
		method: 'GET',
		headers: {}
	};
	...
	this.getCityWeather = function(city) {
		options.path = query_substring + city + format;
		console.log('DEBUG: options.host = ' + options.hostname);
		console.log('DEBUG: options.path = ' + options.path);
		...
		var req = http.request(options, function(res) {
			...
			res.on('data', function(chunk) {
				// Read API responses into a string variable
				msg += chunk;
			});
			res.on('end', function() {
				...
				console.log('DEBUG: result[city] = ' + results[city]);
			});
...
{\n\
  ...
  ,	"current_observation": {\n\
	...
	this.populate = function() {
		...
		this.getCityWeather('CA/Campbell');

		// Get Omaha weather
		this.getCityWeather('NE/Omaha');

		// Get Austin weather
		this.getCityWeather('TX/Austin');

		// Get Timonium weather
		this.getCityWeather('MD/Timonium');
